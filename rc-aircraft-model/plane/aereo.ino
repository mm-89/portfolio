...
// instead of change values in the void()
// TO BE IMPLEMENTED
struct ServoData {
  int inputMin = 0; 
  int inputMax = 1023;
...
void loop() {
  if(Receiver.available()) {
    ...
    aileronR.write(map(controls.aileron, 12, 1023, 10, 160));
    aileronL.write(map(controls.aileron, 5, 1023, 24, 150)); // correct: I inverted the servo for space and balance reasons ;)
    elevator.write(map(controls.elevator, 23, 1023, 0, 180));
  ...
  // force the stall for safety reasons
  // if NO signal is received anymore!!! DONT CANCEL
  if(!isReceiving){
    aileronR.write(0);
    aileronL.write(0);
    elevator.write(0);
  ...
  if(debug) {
      Serial.print("Rece: "); Serial.println(isReceiving); // 1 or 0
    	Serial.print("ailL: "); Serial.print(controls.aileron);
